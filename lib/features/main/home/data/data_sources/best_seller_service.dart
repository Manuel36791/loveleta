...
abstract class BestSellerService {
  Future<List<ProductModel>> getBestSellerProducts(int nextPage);
}

class BestSellerServiceImpl implements BestSellerService {
  @override
  Future<List<ProductModel>> getBestSellerProducts(int nextPage) async {
    Dio dio = await DioFactory.getDio();
    List<ProductModel> productsList = [];

    final categories = await dio.get(
        AppConstants.getBestProductsUri,
        queryParameters: {
          'page': nextPage,
        }
    );
